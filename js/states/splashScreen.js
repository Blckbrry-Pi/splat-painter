...
export default class SplashScreen {
    ...
    update() {
        if ((millis() - this.stateStart > 400 && this.paintSplatters.length < 1) ||
    ...
    draw() {
        ...
        if (millis() - this.stateStart > 2200) {
            let textZoomAnimationValue = map(millis() - this.stateStart, 2200, 2700, 0, 1, true);
            let textMoveAnimationValue = map(millis() - this.stateStart, 3200, 4000, 0, 1, true);
