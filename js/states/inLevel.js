...
export default class InLevel {
    ...
    update() {
        ...
        if (this.levelPlaying.paintballs.length <= 0)
    ...
    generatePaintSplatter(paintball, position) {
        this.paintSplatters.push(new PaintSplatter(position, paintball.size / this.levelPlaying.painting.size * 5, paintball.color));
    }
}
